	...
	implements Searchable280<I>, Dispenser280<I>
{
	 ...
	 * Throws a DuplicateItems exception if 'item' already exists in the tree.
	 */
	public void insert(I item) throws DuplicateItems280Exception {
	 ...
	 */
	protected void insert(I item, AVLTreeNode280<I> parent) throws DuplicateItems280Exception {
		...
		if(item.compareTo(v.item()) == 0) {
			throw new DuplicateItems280Exception("AVL tree does not support storing instances of the same item.");
		}
		else if( item.compareTo(v.item()) < 0) {
			// go left
			if( v.leftNode() == null ) {
				v.setLeftNode(this.createNewNode(item));
			}
			else {
			...
			// go right 
			if( v.rightNode() == null ) {
				v.setRightNode(this.createNewNode(item));
			}
			else {
	...
	protected void delete(I toBeDeleted, AVLTreeNode280<I> parent) {
		...
		if( toBeDeleted.compareTo(this.rootItem()) < 0 ) {
			// Recurse left
			this.rootLeftSubtree().delete(toBeDeleted, this.rootNode());
			...
			// Recurse right
			this.rootRightSubtree().delete(toBeDeleted, this.rootNode());
			...
			if( foundReplacement ) {
				...
				this.delete(itemToBeCopied, parent);
