...
public class Ship {
    ...
    public boolean isOverloaded() {
        ...
        LinkedIterator280 iterator=new LinkedIterator280(this.cargo);
        iterator.goFirst();
        int count=0;
        ...
        cargo.iterator().goFirst();
        double checkweight=0.0;
        for (int i = 0; i <= count ; i++) {

            checkweight+=cargo.iterator().item().weight;
            cargo.iterator().goForth();

        }
        if ((this.capacity)<checkweight){
            return true;
        }
        else {
            return false;
    ...
    public int sacksOfGrainType(Grain type) {
        ...
        int number=0;
        this.cargo.iterator().goFirst();
        System.out.println(this.cargo.iterator().itemExists());

        while(this.cargo.iterator().after()){
            if (cargo.iterator().item().getType()==type){
                number+=1;
            }
            cargo.iterator().goForth();
        }
